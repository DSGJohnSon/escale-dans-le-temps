...
import { headerContent } from '@/data/layout';
...
export function Header() {
	...
	const { branding, navLinks, cta } = headerContent;
	...
	return (
		<header
			className={cn(
				'sticky top-0 z-50 mx-auto w-full max-w-6xl border-b border-transparent md:border md:transition-all md:ease-out',
				{
					'bg-background/95 supports-[backdrop-filter]:bg-background/50 border-border backdrop-blur-lg md:top-4 md:max-w-6xl md:shadow md:rounded-l-full':
						scrolled && !open,
		...
		>
			<nav
				className={cn(
					'flex h-14 w-full items-center justify-between pr-4 md:transition-all md:ease-out',
					{
						'md:pr-2': scrolled,
					},
				)}
			>
				<Link href="/" aria-label={branding.name} className="flex items-center leading-none">
					<Image src="/logo.png" alt='' width={1024} height={1024} className='w-16' />
					<div className='flex flex-col'>
						<span className='font-heading italic font-semibold text-xl'>{branding.name}</span>
						<span className='text-xs'>{branding.tagline}</span>
					</div>
				</Link>
				...
				>
					<MenuToggleIcon open={open} className="size-5" duration={300} aria-hidden="true" />
				</Button>
			</nav>
